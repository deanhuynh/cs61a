...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        def assign_vals(formals, vals):
            if (formals is nil and vals is not nil) or (vals is nil and formals is not nil):
                raise SchemeError('invalid number of arguments')
            elif formals is not nil and vals is not nil:
                child.define(formals.first, vals.first)
                assign_vals(formals.second, vals.second)
        assign_vals(formals, vals)
        # END PROBLEM 11
        return child
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError(ZeroDivisionError)
