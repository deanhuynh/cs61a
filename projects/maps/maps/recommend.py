...
def find_closest(location, centroids):
    ...
    min_centroids_index = 0
    index = 0
    while(index < len(centroids)):
        dist = distance(location, centroids[index])
        if(dist < min_dist):
            min_dist = dist
            min_centroids_index = index
        index += 1
    return centroids[min_centroids_index]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxy = sum([pair[0] * pair[1] for pair in zip([(x - mean(xs)) for x in xs], [(y - mean(ys)) for y in ys])])
    b = sxy / sxx
    a = mean(ys) - b * mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    r_squareds = [pair[1] for pair in pairs]
    predictors = [pair[0] for pair in pairs]

    best_r_squared_index = r_squareds.index(max(r_squareds))
    return predictors[best_r_squared_index]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            rating_dictionary[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
