...
CREATE TABLE schedule AS
  WITH
    flights_helper(flight_path, current, total_flights, cost) as (
      SELECT "SFO", "SFO", 1, 0 FROM flights UNION
      SELECT flight_path || ", " || arrival, arrival, total_flights + 1, cost + price
        FROM flights_helper, flights
        WHERE departure = current and total_flights < 3
    )
  SELECT flight_path, cost FROM flights_helper
    WHERE current = "PDX"
    ORDER BY cost
  ;
...
CREATE TABLE shopping_list AS
  WITH
    best_value(name, category, min) as (
      SELECT name, category, MIN(MSRP/rating)
      FROM products
      GROUP BY category)
  SELECT item, store
  FROM lowest_prices, best_value
  WHERE item = name
  ;
