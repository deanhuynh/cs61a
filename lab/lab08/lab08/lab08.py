...
def add_trees(t1, t2):
    ...
    if len(t1.branches) > len(t2.branches):
        t2.branches += [None for b in range(len(t1.branches) - len(t2.branches))]
    elif len(t1.branches) < len(t2.branches):
        t1.branches += [None for b in range(len(t2.branches) - len(t1.branches))]
    return Tree(t1.label + t2.label, [add_trees(a, b) for a, b in zip(t1.branches, t2.branches)])
